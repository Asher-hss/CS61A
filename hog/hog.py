...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    # BEGIN PROBLEM 6
        say = say(score0, score1) #注意这个不太熟悉的部分，多加理解
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def compare(score0 ,score1):
        pre_score =running_high
        if who:
            if score1 - last_score > pre_score:
                print(score1 - last_score, "point(s)! That's a record gain for Player 1!")
                pre_score = score1 - last_score
            return announce_highest(who, last_score = score1 , running_high = pre_score)
        else:
            if score0 - last_score > pre_score:
                print(score0 - last_score, "point(s)! That's a record gain for Player 0!")
                pre_score = score0 - last_score
            return announce_highest(who, last_score = score0 , running_high = pre_score)
    return compare 
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    n = 0
    for k in range(1,11):
        max = make_averaged(roll_dice, trials_count)(k, dice)
        if max > n:
            n = max
            note = k
    return note
